...
class RoIHeads(torch.nn.Module):
    ...
    def forward(self,
        ...
        # graph_features = box_features      
        
        
        box_features1 = self.g_layer(box_features) 
        # 通过roi_pooling后的两层全连接层
        # box_features_shape: [num_proposals, representation_size]
        box_features2 = self.box_head(box_features)

        if len(box_features1) != 0:
            box_features = box_features2 + box_features1
        else:
            box_features = box_features2
        # 接着分别预测目标类别和边界框回归参数
